...
// Registry 定义一个通用的接口
// 以后万一你想换成 Consul 或 Nacos，只需要实现这俩方法就行，业务代码不用改
...
type EtcdRegistry struct {
	...
	leaseID clientv3.LeaseID // 租约 ID，就像是你的“身份证号”，续租要用
}
...
func (r *EtcdRegistry) Register(ctx context.Context, serviceName string, host string, port int) error {
	// 1. 创建一个租约 (Lease)
	// 5 表示 5 秒。如果 5 秒没续租，Etcd 就会认为你挂了
	...
	// 3. 把数据写入 Etcd，并绑定租约
	// WithLease 意思就是：这个 Key 的寿命跟这个租约绑定
...
func (r *EtcdRegistry) Unregister(ctx context.Context, serviceName string) error {
	// 直接撤销租约
	// 租约一没，绑定的 Key 自动就删除了，不需要手动 Delete Key
