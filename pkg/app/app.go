...
func (a *App) Run() error {
	// 创建一个带取消功能的 Context
	// errgroup 需要这个 context 来通知所有程序“该停了”
	...
	g.Go(func() error {
		...
		// 如果 Start 返回（说明服务挂了），调用 cancel 取消其他任务
		cancel()
		return err
	})
	...
	g.Go(func() error {
		...
		select {
		case <-ctx.Done():
			// 如果 context 已经取消了（说明别的服务挂了），直接退出
			return ctx.Err()
		case sig := <-quit:
			// 如果收到了系统信号
			logger.Log.Info("接收到退出信号，准备优雅关闭...", zap.String("signal", sig.String()))
			// 调用 cancel() 告诉 errgroup ,让errgroup里面所有的任务停止
			cancel()
		}
		return nil
	})
	...
	// Wait 等待所有任务结束
	// 只要有一个任务返回 error，Wait 就返回那个 error
	return g.Wait()
}
